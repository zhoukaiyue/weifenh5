<template>
    ...
    　　　　　　</li> -->
        ...
        <div class="nav-list">
            <ul>
                 <li  v-bind:class="{ select: is_show1}" @click="salesVolume1()">价格
                ...
                <li v-bind:class="{ select: is_show2}" @click="salesVolume2()">订单量<span></span></li>
                <li v-bind:class="{ select: is_show3}" @click="salesVolume3()">库存<span></span></li>
                ...
                <li v-for="(item,index) in datalist">
                    <div class="bb t">
                        ...
                        <div class="goods clerfix" >
                            <h5>{{item.title}}</h5>
                            <p><span class="price">{{item.price}}</span>&ensp;<span class="y-charge">引客价</span>&ensp;<span class="charge">{{item.trueprice}}</span><a href="javascript:"><img src="~@/assets/icon/goods-left.png"></a></p>
                            <ul class="list fl">
                                <li>销量 <span>{{item.xiaoliang}}</span></li>
                                <li>库存 <span>{{item.kucun}}</span></li>
                            </ul>
                            <ul class="list fr">
                                <li>成交订单 <span>{{item.suc}}</span></li>
                                <li>添加时间 <span>{{item.ordertime}}</span></li>
                            </ul>
                        </div>
                    </div>
                    <div class="bottom">
...
.goods {
    ...
    .top{
      ...
      li{display: inline-block;width: 32%;text-align: center;margin-top: 30px;margin-bottom: 20px;p{color: #ffffff;font-size:0.8rem;line-height: 25px;}}
    }      
    /*中间样式*/
    .middle{
        line-height: 30px;border-bottom: 1px solid #d2d2d2;background: #ffffff;
    ...
    /*商品列表*/
    .goods-list{
        li{background: #ffffff;margin-top: 0.8rem;
            .t{padding: 1.5rem 1rem 1rem 1.5rem;}
            .goods-img{
                position: relative;width:30%;display: inline-block;vertical-align:top;img{width: 100%;}
                p{position: absolute;bottom: 0;background: rgba(153,153,153,.7);text-align: center;line-height: 25px;color: #ffffff;font-size: 10px;width: 100%;}
            }
            .goods{
                display: inline-block;margin-left:2%;width: 65%;background: #ffffff;
                h5{width: 95%;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;font-weight: normal;color: #333;}
                p{margin-top: 10px;}
                .price{color: #f54321;font-size:1rem;font-weight: 600;}
                .y-charge{display: inline-block;color:#f54321;border: 1px solid #f54321;padding: 0 3px ;font-size:0.6rem;position: relative;bottom: 0.1rem;} 
                .charge{color: #999999;position: relative;font-size: 0.7rem;}
                .charge:after{content: '';display: inline-block;position: absolute;width: 100%;top: 50%;height: 1px;background: #dddddd;left: 0;}
                .list {
                    display: inline-block;
                    li{line-height: 25px;padding: 0;font-size: 0.7rem;list-style: none;color: #777;margin-top: 0;}
                }
                a{float: right;img{width: 0.5rem;}}
            }
        }
        /*商品列表底部数据*/
        .bottom {
            ...
            li{
                ...
                p{text-align: center;line-height:2rem;}
                img{width: 1.2rem;}
            }
        }
        /*添加商品*/
    ...
    /*添加商品*/
    .add-goods{position: fixed;top: 80%;right: 1rem;width: 5rem;height:5rem;border-radius: 50%;background: #ff8134;text-align: center;z-index:11;img{margin-top: 1rem;}p{color: #ffffff;font-size: 0.9rem;}}
}
</style>
